...
class CartRuleCombinationGet
{
    public static function init ($limit, $offset) {
        $request = 'SELECT * FROM `' . _DB_PREFIX_ . 'cart_rule_combination`  LIMIT '.$limit.' OFFSET '.$offset.';';
        $requestCount = 'SELECT count(*) as count FROM `' . _DB_PREFIX_ . 'cart_rule_combination`;';
